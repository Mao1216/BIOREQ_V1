...
async function loadDatabase() {
    return withLoading(async () => {
        ...
        const productSuppliersResponse = await fetch('/api/bioreq?productSuppliers=1');
        if (productSuppliersResponse.ok) db.productSuppliers = (await productSuppliersResponse.json()).items || [];
...
function renderApprovedProviders() {
    ...
    return `
        <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-white rounded-lg shadow border border-gray-200 overflow-hidden"><div class="overflow-x-auto"><table class="min-w-full divide-y divide-gray-200"><thead class="bg-gray-50"><tr><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">N° Solicitud / Fecha</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Producto / Artículo</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Estado del requerimiento</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Proveedores</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Estado de búsqueda de proveedores</th></tr></thead><tbody class="divide-y divide-gray-200">${approved.length ? approved.map(request => {
...
function renderRequestProviders(requestId) {
    ...
    const catalogProviders = productCode ? db.productSuppliers.filter(provider => provider.productCode === productCode).map(provider => ({ ...provider, isCatalogProvider: true })) : [];
    const registeredNames = new Set(ownProviders.map(provider => String(provider.supplierName || '').trim().toLowerCase()));
    const automaticProviders = catalogProviders.filter(provider => !registeredNames.has(String(provider.supplierName || '').trim().toLowerCase()));
    // Una nueva búsqueda solo parte de los proveedores asociados al producto en el catálogo.
    // Los proveedores de otros requerimientos no se reutilizan automáticamente.
    const providers = [...ownProviders, ...automaticProviders];
    ...
    return `
        <div class="max-w-6xl mx-auto">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-7"><div><button onclick="navigateTo('approved-providers')" class="text-sm text-primary font-medium mb-3"><i class="fas fa-arrow-left mr-1"></i> Aprobadas</button><h1 class="text-2xl font-bold">Proveedores</h1><p class="mt-1 text-sm text-gray-500"><span class="font-semibold text-gray-700">${request.reqNumber}</span> · ${request.productName || '(Sin producto)'}</p><p class="text-sm text-gray-500">${request.requirementDescription || request.description || ''}</p><div class="mt-2">${getSupplierSearchBadge(search.status)}</div></div><div class="flex flex-wrap gap-2">${!isClosed ? `<button onclick="openProviderRegistration('${request.id}')" class="bg-primary hover:bg-primaryHover text-white px-4 py-2 rounded-md shadow text-sm font-medium"><i class="fas fa-plus mr-1"></i> Agregar proveedor</button>` : ''}${mainAction}</div></div>
            ${automaticProviders.length ? '<div class="mb-5 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800"><i class="fas fa-database mr-2"></i>Se muestran únicamente los proveedores asociados a este producto en el catálogo. Puedes añadir alternativas nuevas.</div>' : ''}
...
function renderProviderReview() {
    ...
    const pendingProviders = db.providerRegistrations.filter(provider => ownRequestIds.has(provider.requestId) && provider.status === 'EN EVALUACIÓN DF');
    const requestIds = [...new Set(pendingProviders.map(provider => provider.requestId))];
    const reviewRequests = db.requests.filter(request => requestIds.includes(request.id)).sort((a, b) => new Date(b.updatedAt || b.createdAt) - new Date(a.updatedAt || a.createdAt));
    const selectedRequest = reviewRequests.find(request => request.id === viewContextId) || null;
    const providers = selectedRequest ? pendingProviders.filter(provider => provider.requestId === selectedRequest.id) : [];
    return `<div class="max-w-6xl mx-auto"><div class="mb-6"><h1 class="text-2xl font-bold">Proveedores en revisión</h1><p class="text-sm text-gray-500">Selecciona un requerimiento para evaluar los proveedores enviados por Logística.</p></div><div class="mb-6 rounded-lg border bg-white p-5 shadow-sm"><label for="provider-review-request" class="mb-2 block text-sm font-semibold text-gray-700">Requerimiento</label><select id="provider-review-request" onchange="navigateTo('provider-review', this.value || null)" class="w-full max-w-2xl rounded-md border border-gray-300 bg-white p-2.5 text-sm"><option value="">Seleccione un requerimiento</option>${reviewRequests.map(request => `<option value="${request.id}" ${selectedRequest?.id === request.id ? 'selected' : ''}>${request.reqNumber} - ${request.productName || '(Sin producto)'} (${pendingProviders.filter(provider => provider.requestId === request.id).length} proveedor(es))</option>`).join('')}</select></div>${!reviewRequests.length ? '<div class="rounded-lg border border-dashed bg-white py-14 text-center text-sm text-gray-500">No tienes proveedores pendientes de evaluación.</div>' : !selectedRequest ? '<div class="rounded-lg border border-dashed bg-white py-14 text-center text-sm text-gray-500"><i class="fas fa-hand-pointer mb-3 block text-2xl text-gray-300"></i>Selecciona un requerimiento para ver sus proveedores en revisión.</div>' : `<div class="mb-5 rounded-lg border bg-blue-50 px-5 py-4"><p class="font-semibold text-gray-800">${selectedRequest.reqNumber}</p><p class="text-sm text-gray-600">${selectedRequest.productName || '(Sin producto)'}</p></div><div class="grid grid-cols-1 lg:grid-cols-2 gap-5">${providers.map(provider => `<article class="bg-white rounded-lg border shadow-sm p-5"><div class="flex items-start justify-between gap-3"><div><h2 class="font-semibold text-lg">${provider.supplierName}</h2><p class="text-xs text-gray-500">${provider.manufacturer || 'Fabricante no registrado'} · ${provider.origin || 'Origen no registrado'}</p><p class="mt-1 text-xs text-gray-400">Añadido el ${formatDateTime(provider.createdAt)}</p></div>${getProviderStatusBadge(provider.status)}</div><div class="mt-4 grid grid-cols-2 gap-3 text-sm"><div><p class="text-xs text-gray-500">MOQ / costo</p><p>${(provider.moqs || []).map(item => `${item.quantity || '-'} ${provider.currency || ''} ${item.cost ? `· ${item.cost}` : ''}`).join('<br>') || '-'}</p></div><div><p class="text-xs text-gray-500">Tiempo de envío</p><p>${provider.deliveryTime || '-'}</p></div></div>${providerDocumentChecklist(provider.documentation)}<div class="mt-5 flex flex-wrap justify-end gap-2"><button onclick="reviewProvider('${provider.id}','OBSERVADO')" class="rounded border border-amber-400 px-3 py-2 text-sm text-amber-700">Observar</button><button onclick="reviewProvider('${provider.id}','RECHAZADO')" class="rounded border border-red-400 px-3 py-2 text-sm text-red-700">Rechazar</button><button onclick="reviewProvider('${provider.id}','EN REVISIÓN')" class="rounded bg-primary px-3 py-2 text-sm text-white">Aprobar</button></div></article>`).join('')}</div>`}</div>`;
}
...
function reviewProvider(providerId, status) {
    ...
    openModal(labels[status], `<label class="block text-sm font-medium mb-1">${needsComment ? 'Comentario *' : 'Comentario (opcional)'}</label><textarea id="provider-review-comment" class="w-full border rounded p-2" rows="3" placeholder="${needsComment ? 'Indica el motivo' : 'Comentario de aprobación'}"></textarea>`, async () => {
        ...
        navigateTo('provider-review', provider?.requestId || null);
    }, labels[status]);
}
