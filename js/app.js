...
function renderRequestProviders(requestId) {
    ...
    const registeredNames = new Set([...ownProviders, ...previousProviders].map(provider => String(provider.supplierName || '').trim().toLowerCase()));
    ...
    const providers = [...ownProviders, ...automaticProviders, ...previousProviders.filter(provider => !ownProviders.some(own => own.id === provider.id))];
    ...
    return `
        <div class="max-w-6xl mx-auto">
            ...
            ${(automaticProviders.length || (previousProviders.length && !ownProviders.length)) ? '<div class="mb-5 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800"><i class="fas fa-history mr-2"></i>Se muestran automáticamente los proveedores registrados para este producto. Puedes añadir alternativas nuevas.</div>' : ''}
